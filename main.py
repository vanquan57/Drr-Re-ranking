...
tf.app.flags.DEFINE_string("model_log_dir", "./log/", "the model log directory")
...
def train():
    print("trainig....")
    if not os.path.exists(FLAGS.model_log_dir):
        os.mkdir(FLAGS.model_log_dir)
        print("create log directory:{0}".format(FLAGS.model_log_dir))
    model = get_model()
    ...
    callback_list = [ TensorBoard(log_dir=FLAGS.model_log_dir),
