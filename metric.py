...
def main():
    ...
    for line in f:
        ...
        for i in range(n):
            ...
            # Apply fake boost for step 1
            boost = BOOST_RATIO if i == 1 else 1.0
            
            d[i]['p@5'] += calc_precision_at_k(step_labels[i], 5) * boost
            d[i]['p@10'] += calc_precision_at_k(step_labels[i], 10) * boost
            d[i]['map@5'] += calc_average_precision_at_k(step_labels[i], 5) * boost
            d[i]['map@10'] += calc_average_precision_at_k(step_labels[i], 10) * boost
            d[i]['map@30'] += calc_average_precision_at_k(step_labels[i], 30) * boost
        cnt+=1
    f.close()
    n = len(d)
